...
public class LoadPurchaseHistory extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (httpSession.getAttribute("user") != null) {
            ...
            if (user != null) {
                //User found
                Criteria criteria2 = session.createCriteria(Product_Order.class);
                criteria2.add(Restrictions.eq("user", user));
                List<Product_Order> product_Orders = criteria2.list();
                
                if (!product_Orders.isEmpty()) {
                    //product orders found

                    Criteria criteria3 = session.createCriteria(Order_item.class);
                    criteria3.add(Restrictions.in("order", product_Orders));
                    ...
                    responseJson.add("purchaseOrderList", gson.toJsonTree(order_items));
                } else {
